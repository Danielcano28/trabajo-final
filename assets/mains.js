...
function add_cart_from_product(base_datos) {
  ...
  productsHTML.addEventListener("click", function (e) {
    if (e.target.classList.contains("bx-plus")) {
      ...
      const productFind = base_datos.products.find(
      ...
      if (base_datos.trolley[productFind.id]) {
        ...
        base_datos.trolley[productFind.id] = { ...productFind, amount: 1 };
      }

      window.localStorage.setItem(
        "trolley",
        JSON.stringify(base_datos.trolley)
      );
      PrintProductTrolley(base_datos);
      Printvalues(base_datos);
      amount_trolley(base_datos);
    }
  });
}
...
function buy_trolley(base_datos) {
  ...
  butonValue.addEventListener("click", function () {
    ...
    for (const product of base_datos.products) {
      ...
      if (product.id === productsTrolley?.id) {
        compraProduct.push({
          ...product,
          quantity: product.quantity - productsTrolley.amount,
        });
      } else {
    ...
    window.localStorage.setItem(
      "products",
      JSON.stringify(base_datos.products)
    );
...
function amount_trolley(base_datos) {
  const cantidadProducts = document.querySelector(".cantidad-products");

  let amount = 0;

  for (const product in base_datos.trolley) {
    amount += base_datos.trolley[product].amount;
  }

  cantidadProducts.textContent = amount;
}

async function dataInfo() {
  const base_datos = {
    products:
      JSON.parse(window.localStorage.getItem("products")) || (await products()),
